        ...
        public override ISprite Create(MarioActionState state)
        {
            ...
            switch (state)
            {
                case MarioActionState.Idling:
                    return new StaticSprite(MarioCloneGame.GameContent.Load<Texture2D>("NormalMario"), new Rectangle(0, 0, 16, 16));
                case MarioActionState.Walking:
                    return new AnimatedSprite(MarioCloneGame.GameContent.Load<Texture2D>("NormalMario"), new Rectangle(0, 0, 16, 16),
                        2, 2, 0, 2 * 2);
